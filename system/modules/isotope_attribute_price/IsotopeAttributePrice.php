			...
			foreach($objSource->getAttributes() as $field => $value)
			{
				if (isset($GLOBALS['TL_DCA']['tl_iso_products']['fields'][$field]) && !empty($GLOBALS['TL_DCA']['tl_iso_products']['fields'][$field]['attributes']['options'])) {
					...
					if (($optionsKey !== false) && isset($arrOptions[$optionsKey]['price']))
					{
						...
						# ANFANG
						# Eingefüght für zusätzliche Berechnungen 
						
						$fields = explode("*", preg_replace('/[\s]/', '', $arrOptions[$optionsKey]['price'])); 
                         
						foreach($fields as $field) 
						{ 
						  if($GLOBALS['TL_DCA']['tl_iso_products']['fields'][$field]){ 
						     
						    $sql_result = $this->Database->execute("SELECT $field  
						                                            FROM tl_iso_products 
						                                            WHERE id=".$objSource->pid."  
						                                            LIMIT 1" 
						                                          )->fetchAssoc(); 
						    ...
						    if ( !$sql_result[$field] ) { 
						      $sql_result = $this->Database->execute("SELECT $field  
						                                              FROM tl_iso_products 
						                                              WHERE id=".$objSource->id."  
						                                              LIMIT 1" 
						                                            )->fetchAssoc();                 
						    } 
						
						    $factor = floatval(preg_replace('/[^0-9,\.]/', '', $sql_result[$field])); 
						     
						    if ( $attrPrice != 0 ) { 
						      $attrPrice = $attrPrice * $factor; 
						    } 
						    else { 
						      $attrPrice = $factor; 
						    }                                 
